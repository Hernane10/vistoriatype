...
import { CalendarSimples } from "../components/CalendarSimples";
    ...
    <div className="min-h-full">
      ...
      <div className="topbar px-6 py-5 flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="display text-xl font-bold leading-none">
              Vistor<span style={{ color: "var(--accent)" }}>IA</span>
            </h1>
            <p className="text-xs mt-1 font-semibold" style={{ color: "var(--ink-soft)" }}>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button
            onClick={onNew}
            className="btn-primary rounded-full px-4 py-2.5 flex items-center gap-2 text-sm"
          >
      ...
      <div className="max-w-5xl mx-auto px-6 pt-6">
        <div className="flex items-center gap-2 mb-6 flex-wrap">
          <button
            onClick={() => setTab("vistorias")}
            className={`tab-btn px-4 py-2 text-sm flex items-center gap-1.5 ${
              tab === "vistorias" ? "active" : ""
            }`}
          >
      ...
      {tab === "vistorias" && (
        <div className="max-w-5xl mx-auto px-6 pb-8">
          {/* ===== CALENDÁRIO ===== */}
          {calendarVisible ? (
            <CalendarSimples onHide={toggleCalendar} />
          ) : (
            <button
              onClick={toggleCalendar}
              className="btn-ghost rounded-full px-4 py-2 text-xs flex items-center gap-2 mb-6"
            >
          ...
          {inspections.length === 0 ? (
            ...
            <div className="grid gap-3">
              {inspections.map((insp) => {
                const totalItens =
                  insp.ambientes?.reduce((a, amb) => a + (amb.itens?.length || 0), 0) || 0;
                const avarias =
                  insp.ambientes?.reduce(
                  ...
                  <div key={insp.id} className="card p-4 flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0 cursor-pointer" onClick={() => onOpen(insp.id)}>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span
                          className={`badge ${
                            insp.status === "Finalizada" ? "badge-good" : "badge-neutral"
                          }`}
                        >
                          {insp.status}
                        </span>
                      ...
                      <p className="text-xs mt-1 mono" style={{ color: "var(--ink-soft)" }}>
                        {fmtDate(insp.dataVistoria)} · {insp.vistoriador || "sem vistoriador"} ·{" "}
                        {insp.ambientes?.length || 0} ambientes · {totalItens} itens
                        {avarias > 0 && (
                          <span style={{ color: "var(--bad)" }}> · {avarias} avarias</span>
                        )}
                      </p>
                    </div>
